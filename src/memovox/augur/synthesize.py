...
def _cite(text: str, index: int) -> str:
    """Append ONE citation marker to a claim — the claim is the atomic citable unit, so a
    single trailing ``[n]`` attributes the whole of it. (Free-path claims are single
    sentences anyway — rule-based extraction splits on sentences — so "every sentence
    carries a citation" holds verbatim; a rare multi-sentence LLM-extracted claim is cited
    as one unit. Deliberately NOT split per-sentence: util.split_sentences mis-splits
    abbreviations/decimals — "Dr. Smith ... U.S. ... Fig. 3" — and would inject misleading
    mid-sentence markers into the default extractive synthesis output.)"""
